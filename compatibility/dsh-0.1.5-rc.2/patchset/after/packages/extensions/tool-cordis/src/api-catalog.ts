...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'async approvePlan(caller: Agent, request: ApproveTeamPlanRequest): Promise<TeamPlanApprovalSnapshot>',
        description: 'Commit HUMAN approval of the exact current non-empty structural plan revision.',
        parameters: [{ name: 'caller', description: 'exact live Lead Agent selected by the Web Remote authority.' }, { name: 'request', description: 'revision displayed to the approving HUMAN.' }],
        returns: 'the committed approval snapshot.',
      },
      {
        signature: 'registerApprovalPreflight(preflight: (caller: Agent, view: TeamView) => readonly string[]): () => void',
        description: 'Register one live capability-envelope check at the HUMAN approval boundary.',
        parameters: [{ name: 'preflight', description: 'synchronous check over the exact caller and current Team view.' }],
        returns: 'disposer that removes this approval check.',
      },
      {
        signature: 'async reportWork(caller: Agent, request: ReportTeamWorkRequest): Promise<TeamWorkView>',
        description: 'Report the exact caller\'s latest durable work state.',
        parameters: [{ name: 'caller', description: 'exact live Team member reporting current work.' }, { name: 'request', description: 'durable state, summary, optional reason/task, and affected files.' }],
      ...
      {
        signature: '@Remote(\'approvePlan\') async remoteApprovePlan(agent: Agent, request: ApproveTeamPlanRequest): Promise<TeamPlanApprovalResult>',
        description: 'Approve one exact displayed plan revision through the generated Web Remote API.',
        parameters: [{ name: 'agent', description: 'exact live Lead Agent used as the authority credential.' }, { name: 'request', description: 'revision displayed to the approving HUMAN.' }],
        returns: 'the committed approval or a typed plan conflict/rejection.',
      },
      {
        signature: '@Remote(\'reportWork\') async remoteReportWork(agent: Agent, request: ReportTeamWorkRequest): Promise<TeamWorkMutationResult>',
        description: 'Report caller-bound member work through the generated Web Remote API.',
        parameters: [{ name: 'agent', description: 'exact live Team member reporting current work.' }, { name: 'request', description: 'durable state, summary, optional reason/task, and affected files.' }],
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'ApproveTeamPlanRequest',
    declaration: 'export interface ApproveTeamPlanRequest {\n    readonly approvedRevision: number;\n}',
  },
  {
  ...
  {
    name: 'ReportTeamWorkRequest',
    declaration: 'export interface ReportTeamWorkRequest {\n    readonly state: TeamWorkState;\n    readonly summary: string;\n    readonly reason?: string;\n    readonly taskId?: TeamTaskId;\n    readonly files: readonly string[];\n}',
  },
  {
    name: 'RequestContext',
  ...
  {
    name: 'TeamPlanApprovalResult',
    declaration: 'export type TeamPlanApprovalResult = {\n    readonly ok: true;\n    readonly value: TeamPlanApprovalSnapshot;\n} | {\n    readonly ok: false;\n    readonly error: {\n        readonly code: \'team-plan-conflict\' | \'team-preflight-rejected\' | \'team-rejected\';\n        readonly message: string;\n    };\n};',
  },
  {
    name: 'TeamPlanApprovalSnapshot',
    declaration: 'export interface TeamPlanApprovalSnapshot {\n    readonly approvedRevision: number;\n}',
  },
  {
    name: 'TeamPlanPhase',
    declaration: 'export type TeamPlanPhase = \'draft\' | \'approved\';',
  },
  {
    name: 'TeamTaskAction',
  ...
  {
    name: 'TeamWorkMutationResult',
    declaration: 'export type TeamWorkMutationResult = {\n    readonly ok: true;\n    readonly value: TeamWorkView;\n} | {\n    readonly ok: false;\n    readonly error: {\n        readonly code: \'team-rejected\';\n        readonly message: string;\n    };\n};',
  },
  {
    name: 'TeamWorkSnapshot',
    declaration: 'export interface TeamWorkSnapshot {\n    readonly memberId: SessionId;\n    readonly state: TeamWorkState;\n    readonly summary: string;\n    readonly reason?: string;\n    readonly taskId?: TeamTaskId;\n    readonly files: readonly string[];\n}',
  },
  {
    name: 'TeamWorkState',
    declaration: 'export type TeamWorkState = \'working\' | \'blocked\' | \'review_required\' | \'done\';',
  },
  {
    name: 'TeamWorkView',
    declaration: 'export interface TeamWorkView extends TeamWorkSnapshot {\n    readonly updatedAt: number;\n}',
  },
  {
