...
Human-facing review checklist for evaluating **one AIP artifact** (PLAN / EXEC / LOCAL) against AI Work System MVP v0.3 canonical spec. Complements automated [`lint_aip.py`](../../tooling/lint_aip.py) by adding a semantic review layer that automation cannot enforce.
...
> Legend: `applies_to` = which AIP types the criterion applies to (all = PLAN/EXEC/LOCAL). `lint` = automation coverage (`yes` = fully automated, `partial` = partially, `no` = manual only).
...
| S-02 | `artifact_type` ∈ {aip_plan, aip_exec, aip_local} | all | §5.2-§5.5 | yes | [ ] | [ ] | [ ] | |
...
| S-05 | `status` ∈ {draft, active, done, archived} | all | §5.6 | yes | [ ] | [ ] | [ ] | |
...
| `meta_id` | error | S-03 | |
| `meta_status` | error | S-05 | |
| `meta_plan_source` | warn | S-09 | |
| `section_missing` | error | S-12 / S-13 / S-14 | Alternative names accepted |
