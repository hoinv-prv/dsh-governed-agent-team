...
### Approve and monitor governed work

Only this Web surface can approve a submitted plan. Approval is bound to the displayed plan revision; a structural task edit invalidates it, while status-only progress and member work reports do not. The panel polls at `refreshIntervalMs` only while open, stops on close or disposal, and keeps one bounded refresh in flight. A stall warning after `stallWarningMs` is strict presentation-only inference from unchanged state: it does not write Team state, retry work, run a watchdog, or diagnose a cause. Review-file links navigate through the existing review-file route rather than creating another store, subscription, or viewer.
