...
只有此 Web surface 可以批准已提交 plan。批准绑定到显示的 plan revision；结构性 task edit 会使其失效，而仅 status 的进展变更与 member work report 不会。Panel 仅在打开时按 `refreshIntervalMs` 轮询，关闭或 dispose 后停止，并且最多保留一个有界 refresh。超过 `stallWarningMs` 后的 stall warning 只是根据未变化状态作出的严格 presentation-only inference：它不会写 Team state、重试工作、运行 watchdog 或诊断原因。Review-file link 复用现有 review-file route，而不会创建另一个 store、subscription 或 viewer。
