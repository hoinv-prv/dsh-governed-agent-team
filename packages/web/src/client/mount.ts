...
export const inject = ['sessions', 'remote', 'slots', 'locale', 'sidebarRight']

/** Browser-only dashboard timing configuration. */
export interface Config {
  stallWarningMs: number
  refreshIntervalMs: number
}

/** Defaults preserve a quiet dashboard while still surfacing stale work promptly. */
export const DEFAULT_CONFIG: Config = {
  stallWarningMs: 180_000,
  refreshIntervalMs: 5_000,
}

/** Positive safe-integer validation for all browser timers. */
export const Config: Schema<Partial<Config>, Config> = Schema.object({
  stallWarningMs: Schema.natural().min(1).max(Number.MAX_SAFE_INTEGER).default(DEFAULT_CONFIG.stallWarningMs),
...
function registerUi(ctx: ClientContext, config: Config): void {
  ...
  const actions: TeamActionInjected = {
    ...
    openReviewFile(sessionId, file): void {
      const rootSessionId = leadSessionId(sessionId)
      ctx.sidebarRight.openResource(fileAddressFor(rootSessionId, undefined, file))
    },
  }
  ...
  ctx.slots.inject(
    ...
    () => ctx.slots.register({
      ...
      inject: () => ({
        ...
        openReviewFile: actions.openReviewFile,
 ...
 * @param config - validated Agent Team presentation thresholds.
...
export async function mountAgentTeamUi(
  ...
  const ui = ctx.inject(
    ['sessions', 'remote.agentTeams', 'slots', 'locale', 'sidebarRight'],
    (clientCtx) => { registerUi(clientCtx, config) },
  )
  try {
    await ui
  } catch (error) {
