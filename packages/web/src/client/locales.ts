/** Agent Teams Web dictionaries. */

/** Locale namespace owned by the Agent Teams Web UI. */
export const NS = 'agent-team'

/** Simplified Chinese dictionary and key source. */
export const zh = {
  trigger: 'Agent Team',
  refresh: '刷新 Team',
  close: '关闭',
  loading: '正在加载 Team…',
  empty: '还没有共享任务',
  roster: '成员',
  tasks: '共享任务',
  model: '模型',
  open: '打开 teammate 会话',
  create: '新建任务',
  subject: '任务标题',
  description: '任务描述',
  blockers: '依赖任务 id（逗号分隔）',
  scopes: '写入范围（逗号分隔）',
  save: '保存',
  cancel: '取消',
  edit: '编辑',
  complete: '完成',
  reopen: '重开',
  delete: '删除',
  owner: 'Owner',
  unowned: '未分配',
  blockedBy: '依赖',
  writeScopes: '写入范围',
  ready: '可开始',
  blocked: '被依赖阻塞',
  plan: '计划审批',
  planPhase: '计划状态',
  planRevision: '计划版本',
  approvedRevision: '已批准版本',
  approvePlan: '批准当前计划',
  'planPhase.draft': '草稿',
  'planPhase.approved': '已批准',
  approvalConflict: '计划版本已变化，已重新加载最新计划；请重新检查后批准。',
  conflict: '任务状态已变化，已重新加载；请检查后重试。',
  runtimeStatus: '运行状态',
  durableWorkState: '持久工作状态',
  noDurableWork: '暂无持久工作记录',
  blockerReason: '阻塞原因',
  taskId: '任务 ID',
  updatedAt: '最后持久活动',
  suspectedStall: '疑似停滞',
  openReviewFile: '打开审查文件',
  'workState.working': '工作中',
  'workState.blocked': '已阻塞',
  'workState.review_required': '待审查',
  'workState.done': '已完成',
  'memberStatus.running': '运行中',
  'memberStatus.idle': '空闲',
  'memberStatus.inactive': '未运行',
  'memberStatus.provisioning': '准备中',
  'memberStatus.failed': '失败',
  'status.pending': '待处理',
  'status.in_progress': '进行中',
  'status.completed': '已完成',
} satisfies Record<string, string>

/** Agent Teams locale key union. */
export type TeamKey = keyof typeof zh

/** English dictionary checked against the Chinese key set. */
export const en = {
  trigger: 'Agent Team',
  refresh: 'Refresh Team',
  close: 'Close',
  loading: 'Loading Team…',
  empty: 'No shared tasks yet',
  roster: 'Members',
  tasks: 'Shared tasks',
  model: 'Model',
  open: 'Open teammate conversation',
  create: 'New task',
  subject: 'Task subject',
  description: 'Task description',
  blockers: 'Blocking task ids (comma separated)',
  scopes: 'Write scopes (comma separated)',
  save: 'Save',
  cancel: 'Cancel',
  edit: 'Edit',
  complete: 'Complete',
  reopen: 'Reopen',
  delete: 'Delete',
  owner: 'Owner',
  unowned: 'Unowned',
  blockedBy: 'Blocked by',
  writeScopes: 'Write scopes',
  ready: 'Ready',
  blocked: 'Blocked by dependencies',
  plan: 'Plan approval',
  planPhase: 'Plan status',
  planRevision: 'Plan revision',
  approvedRevision: 'Approved revision',
  approvePlan: 'Approve current plan',
  'planPhase.draft': 'Draft',
  'planPhase.approved': 'Approved',
  approvalConflict: 'The plan changed and the latest revision was reloaded. Review it before approving again.',
  conflict: 'Task state changed and was reloaded. Review it before retrying.',
  runtimeStatus: 'Runtime status',
  durableWorkState: 'Durable work state',
  noDurableWork: 'No durable work reported',
  blockerReason: 'Blocker reason',
  taskId: 'Task ID',
  updatedAt: 'Last durable activity',
  suspectedStall: 'Suspected stall',
  openReviewFile: 'Open review file',
  'workState.working': 'Working',
  'workState.blocked': 'Blocked',
  'workState.review_required': 'Review required',
  'workState.done': 'Done',
  'memberStatus.running': 'Running',
  'memberStatus.idle': 'Idle',
  'memberStatus.inactive': 'Inactive',
  'memberStatus.provisioning': 'Provisioning',
  'memberStatus.failed': 'Failed',
  'status.pending': 'Pending',
  'status.in_progress': 'In progress',
  'status.completed': 'Completed',
} satisfies Record<TeamKey, string>
