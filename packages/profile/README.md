...
The layer adds the Agent Teams domain and its scoped creation, roster, messaging, interruption, waiting, work-status, and task-board tools. It disables the global continuable-child control rows whose tool names overlap with Team controls, while leaving `subagent` and `subagent_fork` available as one-shot delegation tools.

The Tool Team row explicitly uses `freshProvider: spawn`, `forkProvider: fork`, `minExecutionMembers: 2`, and `maxExecutionMembers: 4`. Its exact external restricted-mode allowlist is `ask_user_question`, `glob`, `grep`, `read`, and `read_image`: the four canonical repository read/search tools plus the non-workspace-mutating HUMAN interaction tool visible through the standard Agent preset in the base composition. Team-owned coordination, WBS, and status tools remain internal and are not redundantly allowlisted.
