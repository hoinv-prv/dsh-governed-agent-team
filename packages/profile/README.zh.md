...
本层会添加 Agent Teams domain，以及 Team-scoped 创建、roster、消息、interrupt、等待、work-status 与任务板工具。它会禁用工具名与 Team control 重叠的全局 continuable-child control 行，同时保留 `subagent` 与 `subagent_fork` 作为一次性 delegation 工具。

Tool Team 行显式使用 `freshProvider: spawn`、`forkProvider: fork`、`minExecutionMembers: 2` 与 `maxExecutionMembers: 4`。restricted mode 的精确外部 allowlist 是 `ask_user_question`、`glob`、`grep`、`read` 与 `read_image`：即四个规范 repository 只读／搜索工具，加上 base composition 的标准 Agent preset 中可见且不修改 workspace 的 HUMAN 交互工具。Team 自有 coordination、WBS 与 status 工具保持内部使用，不重复列入 allowlist。
