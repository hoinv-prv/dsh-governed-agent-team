 ...
 */
export function isStructuralTaskMutation(
  prior: TeamTaskSnapshot | undefined,
  next: TeamTaskSnapshot,
): boolean {
  if (prior === undefined) return true
  return prior.subject !== next.subject
    || prior.description !== next.description
    || prior.status !== 'deleted' && next.status === 'deleted'
    || prior.blockedBy.length !== next.blockedBy.length
    || prior.blockedBy.some((id, index) => id !== next.blockedBy[index])
    || prior.writeScopes.length !== next.writeScopes.length
    || prior.writeScopes.some((scope, index) => scope !== next.writeScopes[index])
}
