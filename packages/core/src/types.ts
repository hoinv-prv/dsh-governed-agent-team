...
/** Derived governance phase for the current structural plan revision. */
export type TeamPlanPhase = 'draft' | 'approved'

/** Compare-and-set request for HUMAN approval of the displayed plan revision. */
export interface ApproveTeamPlanRequest {
  readonly approvedRevision: number
}

/** Browser approval result with revision conflicts kept distinct from other Team rejections. */
export type TeamPlanApprovalResult =
...
/** Canonical durable lifecycle of a member's current unit of work. */
export type TeamWorkState = 'working' | 'blocked' | 'review_required' | 'done'

/** Replay-safe event payload for one member's latest reported work. */
export interface TeamWorkSnapshot {
...
/** Read model enriched with authoritative event-envelope time. */
export interface TeamWorkView extends TeamWorkSnapshot {
  readonly updatedAt: number
}

/** Caller-bound request; member identity and event time are derived by TeamService. */
export interface ReportTeamWorkRequest {
  readonly state: TeamWorkState
  readonly summary: string
  readonly reason?: string
  readonly taskId?: TeamTaskId
...
/** Browser-safe member-work mutation result. */
export type TeamWorkMutationResult =
  | { readonly ok: true; readonly value: TeamWorkView }
  | {
    readonly ok: false
    readonly error: {
