...
The exact live root Session is the authority for governance state. The Lead submits a plan; only a Web `approvePlan` call can approve that exact current plan revision, and every structural task change—create, delete, dependency, owner, subject, description, or write-scope change—invalidates the approval as stale. Status-only progress and durable `working`/`blocked`/`review_required`/`done` work reports are non-structural and do not change the approved revision.

Preflight distinguishes durable work state from transient runtime state, requires the configured minimum live membership, and rejects stale or absent approval. Direct or nested Team dispatch fails closed. These checks provide no filesystem lock, watchdog, retry engine, or automatic root-cause diagnosis.
