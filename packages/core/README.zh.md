...
精确 live Root Session 是治理状态的权威来源。Lead 提交 plan；只有 Web `approvePlan` 调用可以批准精确的当前 plan revision。任何结构性任务变更——创建、删除、依赖、owner、subject、description 或 write-scope 变更——都会使批准变为 stale。仅 status 的进展变更以及持久 `working`／`blocked`／`review_required`／`done` 工作报告属于非结构性变更，不会改变已批准 revision。

Preflight 区分持久 work state 与短暂 runtime state，要求达到配置的最少 live member 数，并拒绝缺失或 stale approval。直接或嵌套 Team dispatch 都会 fail closed。这些检查不提供文件系统锁、watchdog、重试引擎或自动 root-cause diagnosis。
