...
Create and validate the WBS, then resolve every preflight diagnostic except the expected below-minimum-member count. The HUMAN approves the current plan revision through Web. The Lead can then spawn teammates; write-capable execution opens only after the minimum durable teammate count and every other preflight requirement are satisfied.
